...
function studentNeedMurajaah(studentId){
  const rs=[...data.records].filter(r=>r.student_id===studentId).sort((a,b)=>String(b.record_date).localeCompare(String(a.record_date)));
  const last=rs[0];
  return !!(last && last.type==="Ziyadah" && last.result==="Tidak Lancar");
}
...
<label>Santri<select name="studentId">${ss.map(s=>`<option value="${s.id}">${esc(s.name)}${studentNeedMurajaah(s.id)?" — 🔄 WAJIB MURAJAAH":""}</option>`).join("")}</select></label>
...
<div id="murajaahNotice" style="display:none;margin-top:10px;padding:10px;border-radius:10px;background:#fff4df;color:#805c19;font-size:13px;font-weight:700">🔒 Santri ini wajib Murajaah sebelum boleh menerima Ziyadah baru.</div>`,
async f=>{
let student=ss.find(s=>s.id===f.studentId.value);
if(!student)return toast("Santri tidak ditemukan.",true);
let locked=studentNeedMurajaah(student.id);
...
note:f.note.value.trim(),record_date:f.date.value,result:f.result.value
});
if(error)return dbError(error,"Gagal menyimpan setoran");
closeModal();await loadData();
toast(f.result.value==="Tidak Lancar"?"Setoran tersimpan. Santri wajib Murajaah.":"Setoran berhasil disimpan");
render()
});
let sel=$("#mf [name=studentId]"),type=$("#mf [name=type]"),notice=$("#murajaahNotice");
function sync(){
let locked=studentNeedMurajaah(sel.value);
if(locked){type.value="Murajaah";type.querySelector('option[value="Ziyadah"]').disabled=true;notice.style.display="block"}
else{type.querySelector('option[value="Ziyadah"]').disabled=false;notice.style.display="none"}
}
sel.onchange=sync;sync();
}
function records(){let ss=visibleStudents(),rs=data.records.filter(r=>ss.some(s=>s.id===r.student_id));return head("Setoran Hafalan","Pantau ziyadah dan murajaah secara terstruktur.",`<button class="btn primary" onclick="recordModal()">＋ Catat Setoran</button>`)+`<section class="panel"><div class="table-wrap"><table><thead><tr><th>Tanggal</th><th>Santri</th><th>Jenis</th><th>Rentang Hafalan</th><th>Nilai</th><th>Hasil</th><th>Catatan</th></tr></thead><tbody>${rs.map(r=>`<tr><td>${r.record_date}</td><td><b>${esc(ss.find(s=>s.id===r.student_id)?.name||"—")}</b></td><td><b style="color:${r.type==="Ziyadah"?"var(--g)":"#a87522"}">${esc(r.type)}</b></td><td>${esc(r.from_surah)} ${esc(r.from_ayat)} → ${esc(r.to_surah)} ${esc(r.to_ayat)}</td><td class="score">${r.score}</td><td><b style="color:${r.result==="Lancar"?"var(--green)":"#a52a21"}">${esc(r.result||"—")}</b></td><td>${esc(r.note||"—")}</td></tr>`).join("")||`<tr><td colspan="7" class="empty">Belum ada setoran.</td></tr>`}</tbody></table></div></section>`}
...
function pdf(){if(!window.jspdf?.jsPDF){let s=document.createElement("script");s.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";s.onload=pdf;document.head.appendChild(s);return}let doc=new jspdf.jsPDF({unit:"mm",format:"a4"}),ss=visibleStudents(),rs=data.records.filter(r=>ss.some(s=>s.id===r.student_id));doc.setFillColor(7,63,54);doc.rect(0,0,210,35,"F");doc.setTextColor(255,255,255);doc.setFontSize(18);doc.text("HALAQOH TAHFIDZ ONLINE",15,14);doc.setFontSize(9);doc.text("REKAP PERKEMBANGAN HAFALAN SANTRI",15,23);doc.setTextColor(35,70,63);doc.setFontSize(10);doc.text("Pengelola : "+profile.name,15,46);doc.text("Tanggal : "+new Date().toLocaleDateString("id-ID"),15,53);doc.text(`Santri : ${ss.length}   Setoran : ${rs.length}   Ziyadah : ${rs.filter(r=>r.type==="Ziyadah").length}   Murajaah : ${rs.filter(r=>r.type==="Murajaah").length}`,15,60);let y=73;doc.setFontSize(8);rs.forEach((r,i)=>{if(y>278){doc.addPage();y=18}let n=ss.find(s=>s.id===r.student_id)?.name||"—";doc.text(`${i+1}. ${n.slice(0,25)} | ${r.type} | ${r.from_surah} ${r.from_ayat} → ${r.to_surah} ${r.to_ayat} | ${r.score}`,12,y);y+=7});doc.setTextColor(120,135,130);doc.text("Dokumen dibuat oleh Aplikasi Halaqoh Tahfidz Online",15,289);doc.save("Rekap-Halaqoh-"+new Date().toISOString().slice(0,10)+".pdf")}
